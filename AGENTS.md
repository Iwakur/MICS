...
Use the legacy material to understand workflows and terminology, not as code to copy. Reimplement accepted behavior with Laravel conventions and confirm ambiguous rules before committing to a schema. The target scope includes staff and users, students, lesson types and plans, monthly student balances, validated payments and expenses, and simple bank-month closing. Keep access roles separate from staff business roles. Derive student debt from `student_months` and validated payments; never store a mutable current-debt total on `students`. Avoid reviving the legacy accounting journal, SQL console, custom bootstrap provisioning, or unnecessary repository layers unless explicitly requested.
