...
/**
 * MICS HUB source: app Http Requests Admin UpdateStaffRequest. See docs/file-reference.md for its full responsibility.
 */
...
class UpdateStaffRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'email' => ['nullable', 'email', 'max:255', Rule::unique((new Staff)->getTable(), 'email')->ignore($staff)],
            ...
            'user_id' => [
                'nullable',
                Rule::exists('users', 'id')->where(function ($query) use ($staff): void {
                    $query->whereNull('staff_id');

                    if ($staff->user) {
                        $query->orWhere('id', $staff->user->id);
                    }
                }),
            ],
        ];
